@extends('home.parent')
<?php 
$title = \App\Components\Tools::sLang('News', '图文素材');
?>
@section('title',$title)
@section('head_js')
@endsection
...
@section('content')
    <div class="content-wrapper">
        <section class="content-header">
            <h1>
              <?php echo \App\Components\Tools::sLang('News', '图文素材');?>
              <small></small>
            </h1>
            <ol class="breadcrumb">
              <li><a href="{{URL('admin')}}"><i class="fa fa-dashboard"></i> <?php echo \App\Components\Tools::sLang('Home page', '首页');?></a></li>
              <li class="active"><?php echo \App\Components\Tools::sLang('News', '图文素材');?></li>
            </ol>
        </section>
                          ...
                          <table id="data_list" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th><?php echo \App\Components\Tools::sLang('Title', '标题');?></th>
                                    <th><?php echo \App\Components\Tools::sLang('Last update time', '最后更新时间');?></th>
...
//提示信息
    ...
    list_ajax = $("#data_list").dataTable({
        ...
        columnDefs: [
            ...
            {
                targets: 3,
                title: puppet_lang == 2 ? "最后更新时间" : "Last update time",
                render: function (data, type, row, meta) {
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    return "<a href='javascript:void(0);' title='删除' aria-label='Delete' data-pjax='0' onclick='actionList("+row.news_id+",-1)'><span class='glyphicon glyphicon-trash'></span></a>";
                }
            },
        ],
        ajax: function (data, callback, settings) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ URL('admin/material/news/list/ajax') }}",
                cache: false, //禁用缓存
                data: param, //传入组装的参数
                dataType: "json",
...
function actionList(id,status){
  var content = (puppet_lang == 2 ? "删除该图文将会连同微信素材中一起删除，是否确认删除？" : "Deleting the news will be deleted together with the WeChat material. Do you want to delete it?");
  var result = confirm(content);
  if(result){
      var url = "{{URL('admin/material/news/delete')}}";
...
function imageSync(){
    var url = "{{URL('admin/material/news/sync')}}";
    var data = {};
    var result = puppet.myajax('post',url,data,false);
    if(result.code == true){
        alert(result.msg);
