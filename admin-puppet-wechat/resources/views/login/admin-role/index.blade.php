@extends('home.parent')
<?php 
  $title = \App\Components\Tools::sLang('Role list', '管理员角色列表');
?>
@section('title',$title)
@section('head_js')
@endsection
...
@section('content')
    <div class="content-wrapper">
        <section class="content-header">
            <h1>
              <?php echo \App\Components\Tools::sLang('Role list', '管理员角色列表');?>
              <small></small>
            </h1>
            <ol class="breadcrumb">
              <li><a href="{{URL('admin')}}"><i class="fa fa-dashboard"></i> <?php echo \App\Components\Tools::sLang('Home page', '首页');?></a></li>
              <li class="active"><?php echo \App\Components\Tools::sLang('Role list', '管理员角色列表');?></li>
            </ol>
        </section>
                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <a href="{{ URL('admin/role/create') }}" class="btn btn-info"><?php echo \App\Components\Tools::sLang('Add Role', '新增管理员角色');?></a>
        ...
        columnDefs: [
            ...
            },
            {
              ...
              render: function (data, type, row, meta) {
                switch (data) {
                  case 1:
                    button_htm = "<a href='javascript:void(0);' class='btn btn-danger btn-xs' onclick='actionList(" + row.admin_role_id + ",0)'>"+(puppet_lang == 2 ? '关闭' : 'Close')+"</a>";
                    return (puppet_lang == 2 ? '开启' : 'Open')+' &nbsp;&nbsp;' + button_htm;
                    break;
                  case 0:
                    button_htm = "<a href='javascript:void(0);' class='btn btn-success btn-xs' onclick='actionList(" + row.admin_role_id + ",1)'>"+(puppet_lang == 2 ? '开启' : 'Open')+"</a>";
            ...
            },
            {
              ...
              render: function (data, type, row, meta) {
                return  "<a href='{{ URL('admin/role/view') }}/"+data.admin_role_id+"' title='查看' aria-label='View' data-pjax='0'><span class='glyphicon glyphicon-eye-open'></span></a>&nbsp;&nbsp;&nbsp;&nbsp;"+
                  ...
                  "<a href='javascript:void(0);' title='删除' aria-label='Delete' data-pjax='0' onclick='actionList("+data.admin_role_id+",-1)'><span class='glyphicon glyphicon-trash'></span></a>";
              }
            },
        ],
...
function actionList(id,status){
    ...
    switch(status){
        case 0:
          var content = (puppet_lang == 2 ? "是否确认关闭该角色，关闭后，该角色权限将不能显示" : "If you confirm that the role is closed, the role permissions will not be displayed after closing.");
          result = confirm(content);
          break;
        case 1:
          var content = (puppet_lang == 2 ? "是否确认启用该角色" : "Are you sure that the role is enabled?");
          result = confirm(content);
          break;
        case -1:
          var content = (puppet_lang == 2 ? "是否确认删除该角色" : "Are you sure you want to delete this role?");
          result = confirm(content);
          break;
        default:
          var content = (puppet_lang == 2 ? "是否确认删除该角色" : "Are you sure you want to delete this role?");
